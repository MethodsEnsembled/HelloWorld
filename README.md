...
- **Abyssal Ascent** (`abyssal-ascent.html`) — a complete reimagining of the scuba/duck/fish concept. One continuous deep-sea ascent: drag to steer a bioluminescent creature rising from the abyss through 5 depth zones. Dodge sharks and anglerfish, collect light orbs to evolve through 5 life stages (Larva → Leviathan), grab golden jellyfish for bonus lives. Procedural soundtrack that shifts with each zone. Reach the surface to win.
- **Scuba, Fish &amp; Duck** (`scuba-duck-fish.html`) — three watery rounds in one game: tap to swim the duck home dodging fish &amp; divers, drag the fish to dodge a dive-raid, then use the arrow buttons to catch every fish. Each round scores points and the total tallies up at the end. Built-in synthesized soundtrack.
